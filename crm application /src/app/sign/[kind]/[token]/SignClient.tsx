...
import EstimateDocument, { BIZ, DOC_CSS, money, ymdLong, resolveBiz, type Doc } from '@/components/EstimateDocument';
...
export default function SignClient({ kind, token }: { kind: 'estimate' | 'invoice'; token: string }) {
  ...
  const b = doc ? resolveBiz(doc.biz) : BIZ;
  ...
  const signSlot = doc && (
    expired ? (
      ...
      <div>
        ...
        <p className="attest">Approved electronically via secure signing link. A permanent copy of this signed document is retained on record by {b.name}.</p>
      </div>
    ) : (
      <div>
        ...
        <p className="attest">By signing, you approve this {label.toLowerCase()} and authorize {b.name} to perform the work described above at the quoted total.</p>
        <p className="preparedby">Prepared by <b>Mico Sanchez</b>, Owner · {b.name}</p>
      </div>
    )
  );
        ...
        {doc && (
          <>
            ...
            <div className="printbar no-print">
              <span>{[b.name, b.tagline].filter(Boolean).join(' · ')}</span>
