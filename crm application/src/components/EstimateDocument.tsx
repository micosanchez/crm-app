...
export interface Biz {
  name?: string | null; tagline?: string | null; phone?: string | null; email?: string | null;
  website?: string | null; area?: string | null; monogram?: string | null;
  licensed_insured?: boolean | null; ein?: string | null;
}
...
/* ---------- Default business identity (fallback when settings not loaded). ---------- */
export const BIZ = {
  ...
  monogram: 'SJH',
  licensed_insured: true as boolean | null,
  ein: null as string | null,
};

/* Merge loaded settings over defaults (only non-empty overrides win) and derive
   the monogram from the business name so it tracks a renamed business. */
export function resolveBiz(biz?: Biz | null) {
  const b = { ...BIZ };
  if (biz) for (const k of Object.keys(b) as (keyof typeof b)[]) {
    const v = biz[k as keyof Biz];
    if (v !== null && v !== undefined && v !== '') (b as Record<string, unknown>)[k] = v;
  }
  const initials = (b.name || '').split(/[^A-Za-z]+/).filter(Boolean).slice(0, 3).map((w) => w[0]).join('').toUpperCase();
  b.monogram = initials || BIZ.monogram;
  return b;
}
...
export default function EstimateDocument({
  doc, kind, payArea, signSlot, hasOnlinePay = false, biz,
}: {
  doc: Doc;
  ...
  const b = resolveBiz(biz ?? doc.biz);
    ...
    <div className="sheet">
      ...
      <div className="head">
        ...
        <div className="contact">
          {b.phone && <span><b>{b.phone}</b></span>}{b.email && <span>{b.email}</span>}{b.website && <span>{b.website}</span>}
        </div>
