...
export default async function ReconciliationPage() {
  ...
  const [{ data: cashAcct }, { data: accounts }, { data: bankTxns }, { data: lines }, { data: recons }] = await Promise.all([
    supabase.from('account_balances').select('*').eq('account_id', cashId).maybeSingle(),
    supabase.from('accounts').select('id,number,name,type,normal_side').eq('is_active', true).order('number'),
    supabase.from('bank_transactions').select('*').eq('account_id', cashId).order('posted_date', { ascending: false }).limit(500),
    supabase.from('journal_lines')
      .select('id,entry_id,debit,credit,journal_entries!inner(entry_date,memo,status)')
      .eq('account_id', cashId).eq('reconciled', false).eq('journal_entries.status', 'posted').limit(500),
    supabase.from('reconciliations').select('*').eq('account_id', cashId).order('period_month', { ascending: false }).limit(12),
  ]);
  ...
  return (
    <ReconciliationWorkspace
      ...
      cashAccount={(cashAcct ?? null) as AccountBalance | null}
