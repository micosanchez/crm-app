...
export async function POST(req: NextRequest) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // Stripe's fee comes off the balance transaction; a payment_processing expense is generated by trigger.
    let fee = 0;
    const sk = process.env.STRIPE_SECRET_KEY;
    if (sk && s.payment_intent) {
      try {
        const pi = await fetch(`https://api.stripe.com/v1/payment_intents/${s.payment_intent}?expand[]=latest_charge.balance_transaction`, { headers: { Authorization: `Bearer ${sk}` } }).then((r) => r.json());
        fee = Number(pi?.latest_charge?.balance_transaction?.fee ?? 0) / 100;
      } catch { /* fee stays 0; fixable later with update_expense */ }
    }
